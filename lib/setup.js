...
module.exports.getSetupForData = async ({ chalk, data, inquirer, model, setupData }) => {
    ...
    const answers = await inquirer.prompt([
        {
            type: 'list',
            name: 'format',
            message: 'Choose a format for the file where the data objects will be stored:',
            choices: [
                {
                    name: 'JSON',
                    value: 'json'
        ...
        },
        {
            ...
            choices: (model.fieldNames || []).map(fieldName => {
                const example = exampleFieldValues[fieldName] ? ` (e.g. ${exampleFieldValues[fieldName]})` : '';
...
module.exports.getSetupForPage = async ({ chalk, data, inquirer, model, setupData }) => {
    ...
    if (pageType === 'single') {
        ...
        answers.location = locationSource === 'static' ? { fileName: locationValue } : { fileNameField: locationValue };
    } else {
        const { directory, fileNameField } = await inquirer.prompt([
            {
                ...
                choices: [`content/${model.modelName}`, , new inquirer.Separator(), { name: 'Other', value: undefined }].filter(Boolean)
            },
            {
                type: 'input',
                name: 'directory',
    ...
    const { addDateField, contentField, layout, layoutSource } = await inquirer.prompt([
        {
            type: 'list',
            name: 'layoutSource',
        ...
        },
        {
            type: 'confirm',
            name: 'addDateField',
            message: `Do you want to add a 'date' field to the frontmatter? ${chalk.reset.dim(`(e.g. field: 2019-12-31)`)}`,
            default: true
        },
        {
            type: 'list',
